...
function user_list()
{
    ...
    if ($result === false)
    {
     ...
     /*  $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
			...
			$sql = "SELECT * FROM ".$GLOBALS['ecs']->table('users')." as eu right join wxch_user as wu on wu.wxid = eu.wxid  ".$ex_where." and wu.affiliate = ".$id." ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
