...
/**
 * iTaoWuJi 动态内容函数库
 * $Id: lib_insert.php 17217 2016-11-19 06:29:08Z $
 */
if (!defined('IN_ECS')) {
	die('Hacking attempt');
}
...
function insert_history() {
	...
	if (!empty($_COOKIE['ECS']['history'])) {
		...
		$sql = 'SELECT goods_id, goods_name, goods_thumb, shop_price,promote_price, promote_start_date, promote_end_date FROM ' . $GLOBALS['ecs']->table('goods') . " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0";
...
function insert_index_history() {
	...
	if (!empty($_COOKIE['ECS']['history'])) {
		$where = db_create_in($_COOKIE['ECS']['history'], 'goods_id');
		$sql = 'SELECT goods_id, goods_name, goods_thumb, shop_price,market_price, promote_price, promote_start_date, promote_end_date FROM ' . $GLOBALS['ecs']->table('goods') . " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0";
...
function insert_cart_info() {
	$sql = 'SELECT c.*,g.goods_name,g.goods_thumb,g.goods_id,c.goods_number,c.goods_price' . ' FROM ' . $GLOBALS['ecs']->table('cart') . " AS c " . " LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON g.goods_id=c.goods_id " . " WHERE c.user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
	...
	$sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' . ' FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
