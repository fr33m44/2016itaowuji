...
/**
 * iTaoWuJi 商品详情
 * $Id: goods.php 17217 2016-11-19 06:29:08Z $
 */
define('IN_ECS', true);
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price') {
	...
	$goods_attr = str_replace(',', '|', $_REQUEST['attr']);
	...
	if ($goods_id == 0) {
		...
		//hjq 获取库存
		$sql = "SELECT product_number " . 'FROM ' . $GLOBALS['ecs']->table('products') . "WHERE goods_attr = '" . $goods_attr . "'";
		$goods_number = $GLOBALS['db']->getOne($sql);
		if(empty($goods_number))
			$res['stock'] = 0;
		else
			$res['stock'] = $goods_number;
		$res['name'] = $attr_id;
	}
	die($json->encode($res));
}
