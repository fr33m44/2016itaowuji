...
if ($_REQUEST['step'] == 'add_to_cart') {
	...
	if (!empty($goods->spec)) {
		...
		foreach ($goods_attr_id as $key => $value) {
			$sql = "SELECT *" . ' FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' . "WHERE g.goods_attr_id = '" . $value . "'  AND a.attr_type = 1";
		...
		$sql = "SELECT product_number " . 'FROM ' . $GLOBALS['ecs']->table('products') . "WHERE goods_attr = '" . $goods_attr_id . "'";
...
} elseif ($_REQUEST['step'] == 'del_in_cart_combo') //删除购物车项目 by mike
{
	...
	if ($goods->parent == 0) {
		//更新临时购物车（删除基本件）
		$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE user_id='" . $_SESSION['user_id'] . "'" . " AND goods_id = '" . $goods->goods_id . "' AND group_id = '" . $goods->group . "'";
		$GLOBALS['db']->query($sql);
		//更新临时购物车（删除配件）
		$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE user_id='" . $_SESSION['user_id'] . "'" . " AND parent_id = '" . $goods->goods_id . "' AND group_id = '" . $goods->group . "'";
		$GLOBALS['db']->query($sql);
	} else {
		//更新临时购物车（删除配件）
		$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE user_id='" . $_SESSION['user_id'] . "'" . " AND goods_id = '" . $goods->goods_id . "' AND group_id = '" . $goods->group . "'";
...
} elseif ($_REQUEST['step'] == 'add_to_cart_group') //套餐添加到购物车 by mike
{
	...
	//批量加入购物车
	$sql = "SELECT rec_id FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE user_id = '" . $_SESSION['user_id'] . "'" . " AND group_id = '" . $group . "' ORDER BY parent_id limit 1";
	$res = $GLOBALS['db']->query($sql);
	if ($res) {
		//清空购物车中的原有数据
		$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE " . " user_id='" . $_SESSION['user_id'] . "' AND group_id = '" . $group . "'";
		$GLOBALS['db']->query($sql);
		//插入新的数据
		$sql = "INSERT INTO " . $GLOBALS['ecs']->table('cart') . " SELECT * FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE " . " user_id='" . $_SESSION['user_id'] . "' AND group_id = '" . $group . "'";
		$GLOBALS['db']->query($sql);
		//插入更新购物车商品数量
		$sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " set goods_number = '$goods->number' WHERE " . " user_id='" . $_SESSION['user_id'] . "' AND group_id = '" . $group . "'";
		$GLOBALS['db']->query($sql);
		//清空套餐临时数据
		$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE " . " user_id='" . $_SESSION['user_id'] . "' AND group_id = '" . $group . "'";
		$GLOBALS['db']->query($sql);
	} else {
		$result['error'] = 1;
	...
	*/
	if ($_SERVER['REQUEST_METHOD'] == 'GET') {
		...
		$sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' AND is_gift > 0";
		if ($db->getOne($sql) > 0) {
	...
	*/
	...
	$p = $GLOBALS['db']->getOne("SELECT region_name FROM " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '" . $consignee['province'] . "'");
	$c = $GLOBALS['db']->getOne("SELECT region_name FROM " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '" . $consignee['city'] . "'");
	$d = $GLOBALS['db']->getOne("SELECT region_name FROM " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '" . $consignee['district'] . "'");
	$consignee['address'] = $p . $c . $d . $consignee['address'];
...
elseif ($_REQUEST['step'] == 'done') {
	...
	*/
	...
	/* 插入订单商品 */
	$sql = "INSERT INTO " . $ecs->table('order_goods') . "( " . "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) " . " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id" . " FROM " . $ecs->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$flow_type'";
	$db->query($sql);
	/* 修改拍卖活动状态 */
	...
	*/
	...
	/* 如果订单金额为0 处理虚拟卡 */
	if ($order['order_amount'] <= 0) {
		$sql = "SELECT goods_id, goods_name, goods_number AS num FROM " . $GLOBALS['ecs']->table('cart') . " WHERE is_real = 0 AND extension_code = 'virtual_card'" . " AND user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$flow_type'";
		$res = $GLOBALS['db']->getAll($sql);
		$virtual_goods = array();
		foreach ($res AS $row) {
...
elseif ($_REQUEST['step'] == 'ajax_update_cart') {
	...
	if (isset($_POST['rec_id']) && isset($_POST['goods_number'])) {
		...
		//查询：
		$sql = "SELECT `goods_id`, `goods_attr_id`,`product_id`, `extension_code` FROM" . $GLOBALS['ecs']->table('cart') . " WHERE rec_id='$key' AND user_id='" . $_SESSION['user_id'] . "'";
		...
		if ($val > 0) {
			...
			while ($offers_accessories_row = $GLOBALS['db']->fetchRow($offers_accessories_res)) {
				if ($row_num > $val) {
					$sql = "DELETE FROM" . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' " . "AND rec_id ='" . $offers_accessories_row['rec_id'] . "' LIMIT 1";
					$GLOBALS['db']->query($sql);
				}
				$row_num++;
			}
			...
			if ($goods['extension_code'] == 'package_buy') {
				//更新购物车中的商品数量
				$sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_number= '$val' WHERE rec_id='$key' AND user_id='" . $_SESSION['user_id'] . "'";
			}
			/* 处理普通商品或非优惠的配件*/
			else {
				...
				//更新购物车中的商品数量
				$sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_number= '$val', goods_price = '$goods_price' WHERE rec_id='$key' AND user_id='" . $_SESSION['user_id'] . "'";
			}
		}
		//订货数量等于0
		else {
			...
			$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE rec_id='$key' AND user_id='" . $_SESSION['user_id'] . "'";
		}
		$GLOBALS['db']->query($sql);
		/* 删除所有赠品*/
...
elseif ($_REQUEST['step'] == 'add_favourable') {
	...
	if ($favourable['act_type'] == FAT_GOODS) {
		...
		/* 检查是否已在购物车 */
		$sql = "SELECT goods_name" . " FROM " . $ecs->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "'" . " AND rec_type = '" . CART_GENERAL_GOODS . "'" . " AND is_gift = '$act_id'" . " AND goods_id " . db_create_in($_POST['gift']);
	...
	$sql = "DELETE FROM " . $ecs->table('cart') . " WHERE user_id='" . $_SESSION['user_id'] . "'";
	$db->query($sql);
	ecs_header("Location:./\n");
} elseif ($_REQUEST['step'] == 'drop_to_collect') {
	if ($_SESSION['user_id'] > 0) {
		$rec_id = intval($_GET['id']);
		$goods_id = $db->getOne("SELECT  goods_id FROM " . $ecs->table('cart') . " WHERE rec_id = '$rec_id' AND user_id = '" . $_SESSION['user_id'] . "' ");
...
elseif ($_REQUEST['step'] == 'add_package_to_cart') {
	...
	/* 购物车中商品配件列表 */
	//取得购物车中基本件ID
	$sql = "SELECT goods_id " . "FROM " . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' " . "AND rec_type = '" . CART_GENERAL_GOODS . "' " . "AND is_gift = 0 " . "AND extension_code <> 'package_buy' " . "AND parent_id = 0 ";
...
function flow_available_points() {
	$sql = "SELECT SUM(g.integral * c.goods_number) " . "FROM " . $GLOBALS['ecs']->table('cart') . " AS c, " . $GLOBALS['ecs']->table('goods') . " AS g " . "WHERE c.user_id = '" . $_SESSION['user_id'] . "' AND c.goods_id = g.goods_id AND c.is_gift = 0 AND g.integral > 0 " . "AND c.rec_type = '" . CART_GENERAL_GOODS . "'";
...
function flow_update_cart($arr) {
	...
	foreach ($arr AS $key => $val) {
		...
		if ($val > 0) {
			...
			while ($offers_accessories_row = $GLOBALS['db']->fetchRow($offers_accessories_res)) {
				if ($row_num > $val) {
					$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' " . "AND rec_id = '" . $offers_accessories_row['rec_id'] . "' AND group_id='' LIMIT 1"; //by mike
					$GLOBALS['db']->query($sql);
				}
				$row_num++;
			}
			...
			if ($goods['extension_code'] == 'package_buy') {
				//更新购物车中的商品数量
				$sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_number = '$val' WHERE rec_id='$key' AND user_id='" . $_SESSION['user_id'] . "' AND group_id=''"; //by mike
			...
			/* 处理普通商品或非优惠的配件 */
				...
				$goods_price = get_final_price($goods['goods_id'], $val, true, $attr_id);
				//更新购物车中的商品数量
				$sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_number = '$val', goods_price = '$goods_price' WHERE rec_id='$key' AND user_id='" . $_SESSION['user_id'] . "' AND group_id=''"; //by mike
		...
		//订货数量等于0
			...
			$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE rec_id='$key' AND user_id='" . $_SESSION['user_id'] . "' AND group_id=''"; //by mike
...
function flow_cart_stock($arr) {
	foreach ($arr AS $key => $val) {
		...
		$sql = "SELECT `goods_id`, `goods_attr_id`, `extension_code` FROM" . $GLOBALS['ecs']->table('cart') . " WHERE rec_id='$key' AND user_id='" . $_SESSION['user_id'] . "'";
		$goods = $GLOBALS['db']->getRow($sql);
...
function flow_drop_cart_goods($id) {
	...
	if ($row) {
		...
		if ($row['extension_code'] == 'package_buy') {
			$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' " . "AND rec_id = '$id' LIMIT 1";
		}
		...
		elseif ($row['parent_id'] == 0 && $row['is_gift'] == 0) {
			...
			$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' " . "AND (rec_id IN ($_del_str) OR parent_id = '$row[goods_id]' OR is_gift <> 0) AND group_id='" . $row['group_id'] . "'";
		}
		//如果不是普通商品，只删除该商品即可
		else {
			$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' " . "AND rec_id = '$id' LIMIT 1";
...
function cart_favourable() {
	$list = array();
	$sql = "SELECT is_gift, COUNT(*) AS num " . "FROM " . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "'" . " AND rec_type = '" . CART_GENERAL_GOODS . "'" . " AND is_gift > 0" . " GROUP BY is_gift";
	$res = $GLOBALS['db']->query($sql);
	while ($row = $GLOBALS['db']->fetchRow($res)) {
...
function add_gift_to_cart($act_id, $id, $price) {
	$sql = "INSERT INTO " . $GLOBALS['ecs']->table('cart') . " (" . "user_id, user_id, goods_id, goods_sn, goods_name, market_price, goods_price, " . "goods_number, is_real, extension_code, parent_id, is_gift, rec_type ) " . "SELECT '$_SESSION[user_id]', '" . $_SESSION['user_id'] . "', goods_id, goods_sn, goods_name, market_price, " . "'$price', 1, is_real, extension_code, 0, '$act_id', '" . CART_GENERAL_GOODS . "' " . "FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = '$id'";
	$GLOBALS['db']->query($sql);
}
/**
 * 添加优惠活动（非赠品）到购物车
...
function add_favourable_to_cart($act_id, $act_name, $amount) {
	$sql = "INSERT INTO " . $GLOBALS['ecs']->table('cart') . "(" . "user_id, user_id, goods_id, goods_sn, goods_name, market_price, goods_price, " . "goods_number, is_real, extension_code, parent_id, is_gift, rec_type ) " . "VALUES('$_SESSION[user_id]', '" . $_SESSION['user_id'] . "', 0, '', '$act_name', 0, " . "'" . (-1) * $amount . "', 1, 0, '', 0, '$act_id', '" . CART_GENERAL_GOODS . "')";
	$GLOBALS['db']->query($sql);
}
/**
 * 取得购物车中某优惠活动范围内的总金额
...
function cart_favourable_amount($favourable) {
	/* 查询优惠范围内商品总额的sql */
	$sql = "SELECT SUM(c.goods_price * c.goods_number) " . "FROM " . $GLOBALS['ecs']->table('cart') . " AS c, " . $GLOBALS['ecs']->table('goods') . " AS g " . "WHERE c.goods_id = g.goods_id " . "AND c.user_id = '" . $_SESSION['user_id'] . "' " . "AND c.rec_type = '" . CART_GENERAL_GOODS . "' " . "AND c.is_gift = 0 " . "AND c.goods_id > 0 ";
	/* 根据优惠范围修正sql */
