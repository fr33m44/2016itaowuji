...
elseif ($action == 'act_register') {
	...
	if ($_CFG['shop_reg_closed']) {
		...
		if (empty($_POST['agreement'])) {
			$res = array('err' => 1, 'msg' => $_LANG['passport_js']['agreement']);
			die($json->encode($res));
		}
		if (empty($qrm)) {
			$res = array('err' => 2, 'msg' => '确认码不能为空');
			die($json->encode($res));
		}
		...
		if (strlen($password) < 6) {
			$res = array('err' => 4, 'msg' => $_LANG['passport_js']['password_shorter']);
			die($json->encode($res));
		}
		if (strpos($password, ' ') > 0) {
			$res = array('err' => 5, 'msg' => $_LANG['passwd_balnk']);
			die($json->encode($res));
		}
		/* 验证码检查 */
		if ((intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0) {
			if (empty($_POST['captcha'])) {
				$res = array('err' => 6, 'msg' => '未填写验证码');
				die($json->encode($res));
			}
			/* 检查验证码 */
			...
			if (!$validator->check_word($_POST['captcha'])) {
				$res = array('err' => 7, 'msg' => $_LANG['invalid_captcha']);
				die($json->encode($res));
			}
		}
		/*手机确认码检查*/
		...
		$count = $db->getOne($sql);
		if (!empty($count)) {
			$res = array('err' => 9, 'msg' => '该手机号已经被注册。');
			die($json->encode($res));
		}
		/*判断手机号是否已经存在*/
		$sql = "select count(1) from " . $ecs->table("users") . " where user_name='$username'";
		$count = $db->getOne($sql);
		if (!empty($count)) {
		...
		if (register($username, $password, $email, $other) !== false) {
			...
			$address = array('default' => 1, //default>0的时候将user_address.address_id赋值到users.address_id 默认地址为店铺地址
			'user_id' => $_SESSION['user_id'], 'country' => 1, 'province' => $province, 'city' => $city, 'district' => $district, 'address' => $address, 'consignee' => $shop_name, 'shop_type' => $shop_type, //店铺类型
			'mobile' => $other['mobile_phone']);
...
elseif ($action == 'is_registered') {
	...
	if ($user->check_user($username) || admin_registered($username)) { //已经注册
		echo 'false';
	} else {
		echo 'true';
	}
...
elseif ($action == 'act_login') {
	...
	if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0) {
		...
		if (!$validator->check_word($_POST['captcha'])) {
			$res = array('err' => 2, 'msg' => $_LANG['invalid_captcha']);
			die($json->encode($res));
		}
	}
	if ($user->login($username, $password, isset($_POST['remember']))) {
		...
		$res = array('err' => 0, 'msg' => '登陆成功');
		die($json->encode($res));
	} else {
		$_SESSION['login_fail']++;
		$res = array('err' => 3, 'msg' => $_LANG['login_failure']);
		die($json->encode($res));
	}
}
/* 处理 ajax 的发送手机确认码请求 */
elseif ($action == 'getqrm') {
	...
	do {
		...
		if (!$validator->check_word($_GET['captcha'])) {
			$result = array('error' => 1, 'content' => '您输入的验证码不正确');
			break;
		}
		/*检查手机号格式*/
...
elseif ($action == 'profile') {
	...
	foreach ($consignee_list as $key => $val) {
		if ($key != 0) unset($consignee_list[$key]);
	}
