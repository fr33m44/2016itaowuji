...
function get_consignee_list($user_id) {
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') . " WHERE user_id = '$user_id'  order by address_id LIMIT 5 ";
...
function add_bonus($user_id, $bouns_sn) {
	...
	/* 查询红包序列号是否已经存在 */
	$sql = "SELECT bonus_id, bonus_sn, user_id, bonus_type_id FROM " . $GLOBALS['ecs']->table('user_bonus') . " WHERE bonus_sn = '$bouns_sn'";
...
function get_user_orders($user_id, $num = 10, $start = 0) {
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$arr[] = array('order_id' => $row['order_id'], 'order_sn' => $row['order_sn'], 'order_time' => local_date($GLOBALS['_CFG']['time_format'], $row['add_time']), 'order_status' => $row['order_status'], 'total_fee' => price_format($row['total_fee'], false), 'handler' => $row['handler']);
	}
	return $arr;
}
/**
...
function update_address($address) {
	...
	if (isset($address['default']) && $address['default'] > 0 && isset($address['user_id'])) {
		$sql = "UPDATE " . $GLOBALS['ecs']->table('users') . " SET shop_address_id = '" . $address_id . "' " . " WHERE user_id = '" . $address['user_id'] . "'";
...
function return_to_cart($order_id) {
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		//检查商品价格是否有会员价格
		$sql = "SELECT goods_number FROM" . $GLOBALS['ecs']->table('cart') . " " . "WHERE user_id = '" . $_SESSION['user_id'] . "' " . "AND goods_id = '" . $row['goods_id'] . "' " . "AND rec_type = '" . CART_GENERAL_GOODS . "' LIMIT 1";
		...
		// 返回购物车：看有没有相同商品
		$sql = "SELECT goods_id " . "FROM " . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' " . " AND goods_id = '$return_goods[goods_id]' " . " AND goods_attr = '$return_goods[goods_attr]' " . " AND parent_id = '$return_goods[parent_id]' " . " AND is_gift = 0 " . " AND rec_type = '" . CART_GENERAL_GOODS . "'";
	...
	// 清空购物车的赠品
	$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' AND is_gift = 1";
	$GLOBALS['db']->query($sql);
	return true;
}
/**
