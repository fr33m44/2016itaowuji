    ...
    public function responseMsg($db, $user, $base_url){
        ...
        if (!empty($postStr) or $debug == 1){
            ...
            if(empty($fromUsername)){
                $fromUsername = 'oIM-ajl04bGTTtw7yQfEImx-tJH4';
            }
            $time = time();
            $lang = array();
...
( '$fromUsername', '$keyword', $time);");
            ...
            if($postObj -> Event == 'subscribe'){
                if(empty($postObj -> EventKey)){
                    ...
                    $scan_ret = $db -> getRow("SELECT function FROM `wxch_qr` WHERE scene_id =$scene_id");
            ...
            if($keyword == 'bd'){
                ...
                if($ArticleCount >= 1){
                    foreach($ret as $v){
                        ...
                        if($oauth_state == 'true'){
                            $goods_url = $oauth_location . $m_url . 'goods.php?id=' . $v['goods_id'];
            ...
            if(!empty($keyword)){
                ...
                if($ArticleCount >= 1){
                 ...
                 <Url><![CDATA[" . $goods_url . "]]></Url>
                    ...
                    $thistable = $db -> prefix . 'goods';
                    if($plustj == 'true'){
                        $tj_str = $this -> plusTj($db, $m_url, $postfix, $oauth_location, $oauth_state, $goods_is);
...
('$fromUsername', '$w_message', '$belong', '$time');";
    ...
    protected function plusTj($db, $m_url, $postfix, $oauth_location, $oauth_state, $goods_is){
        $thistable = $db -> prefix . 'goods';
        ...
        if($tj_goods['goods_id']){
            if($oauth_state == 'true'){
                return $tj_str = "\r\n我们为您推荐:" . "<a href='$oauth_location" . "$m_url" . 'goods.php?id=' . $tj_goods['goods_id'] . "'>$tj_goods[goods_name]</a>";
            }elseif($oauth_state == 'false'){
                return $tj_str = "\r\n我们为您推荐:" . '<a href="' . $m_url . 'goods.php?id=' . $tj_goods[goods_id] . $postfix . '">' . $tj_goods[goods_name] . '</a>';
            }
        }
    }
...
nation=大雁塔&mode=driving&region=西安&output=html';
    ...
    public function mydebug($textTpl, $fromUsername, $toUsername, $time, $contents){
        if($fromUsername == 'oIM-ajl04bGTTtw7yQfEImx-tJH4'){
            $msgType = "text";
            $contentStr = $contents;
            $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
            echo $resultStr;
    ...
    public function mechat($post_str){
        ...
        $msgId = $postObj -> MsgId;
        $post_arr = array('ToUserName' => $toUsername, 'FromUserName' => $fromUsername, 'CreateTime' => '', 'MsgType' => $msgType, 'Content' => $Content, 'MsgId' => $msgId);
