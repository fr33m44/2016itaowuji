...
if ($_REQUEST['act'] == 'list') {
    ...
    if (empty($name) || !is_array($name)) {
        $link[0] = array(
            'href' => 'website.php?act=list',
            'text' => $_LANG['webstte_list']
        );
    ...
    if (file_exists($filepath . 'config/' . $type . '_config.php')) {
        include_once ($filepath . 'config/' . $type . '_config.php');
        if (!defined(RANK_ID)) $db->query('DELETE FROM ' . $ecs->table('user_rank') . ' WHERE `rank_id`=\'' . RANK_ID . '\'');
